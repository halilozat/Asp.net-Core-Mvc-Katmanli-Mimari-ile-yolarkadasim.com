$(document).ready(function () {
   ...
   const dataTable = $('#journeysTable').DataTable({
        dom:
    ...
    $(document).on('click',
        ...
        function (event) {
            ...
            const journeyTitle = tableRow.find('td:eq(5)').text();
            Swal.fire({
                title: 'Silmek istediğinize emin misiniz?',
                text: `${journeyTitle} başlıklı ilan silinicektir!`,
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        success: function (data) {
                            const journeyResult = jQuery.parseJSON(data);
                            if (journeyResult.ResultStatus === 0) {
                                Swal.fire(
                                    'Silindi!',
