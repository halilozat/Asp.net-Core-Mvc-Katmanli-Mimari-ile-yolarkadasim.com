    ...
    $(function () {
        ...
        $('#btnAdd').click(function () {
            $.get(url).done(function (data) {
                ...
                placeHolderDiv.find(".modal").modal('show'); /*.modal divini bul, modal yap, göster*/
            });
        });
        ...
        placeHolderDiv.on('click',
            '#btnSave',
            function (event) {
                event.preventDefault();  /*butonun kendi click işlemini engelledik*/
                const form = $('#form-category-add'); /*göndereceğimiz formu seçtik (modal formun id'si)*/
                const actionUrl = form.attr('action'); /*formun asp-action'unu aldık*/
                const dataToSend = form.serialize(); /*formun içerisindeki veriyi aldık (formdaki veriyi categoryadddto olarak dönüştürdük.)*/
                $.post(actionUrl, dataToSend).done(function (data) { /*verileri gerekli url'e post etme işlemleri*/
                    console.log(data);
                    ...
                    if (isValid) { /*isValid true ise model durumumuz doğru, işlemler başarılı, modal formu kapat, yeni kaydı ekle, toastr mesajını ver diyeceğiz.*/
                        placeHolderDiv.find('.modal').modal('hide'); /*modal formu gizle*/
                        const newTableRow = `
                                <tr name="${categoryAddAjaxModel.CategoryDto.Category.Id}">
                                                ...
                                                </tr>`;/*yeni bir tablerow*/
                        const newTableRowObject = $(newTableRow);/*tablerowu obje haline getir.*/
                        newTableRowObject.hide();
                        $('#categoriesTable').append(newTableRowObject);
                        newTableRowObject.fadeIn(3500); /*3.5 snde açıl*/
                        ...
                        toastr.warning(summaryText); /*eksik değer girildiğinde uyarı toastr'ı gelecek.*/
                    }
                });
            });
    });
